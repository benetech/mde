<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:import href="mde_Algebra1Text_DelimitedGraphFeatures.xsl" />
	...
	<xsl:template name="lineSpecifics">
		...
		<xsl:apply-templates select="inclination">
			<xsl:with-param name="rad">
				<xsl:value-of select="incrad/decimalValue" />
			</xsl:with-param>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:template name="polarLemniscateSpecifics">
		The graph consists of two loops located symmetrically on opposite
		sides of the
	...
	<xsl:template name="cardioidSpecifics">
		...
		direction from the cleft of the heart
		to its point most distant from the center. This axis is inclined at an
		angle of
		<xsl:call-template name="angleModel">
	...
	<xsl:template name="loopWithinALoopSpecifics">
		...
		point at the origin through the smaller loop to the most distant point on
		the curve.
		...
		. The size of the smaller loop is
		<xsl:call-template name="numberModel">
	...
	<xsl:template name="eccentricCircleSpecifics">
		...
		It is symmetric about an axis which runs
		<xsl:value-of select="axis" />
		from the
		...
		. The elongated end is a distance of
		<xsl:call-template name="numberModel">
			<xsl:with-param name="featureName" select="maxLength" />
		</xsl:call-template>
		from the origin, and the closest point on the curve is a
		distance of
		<xsl:call-template name="numberModel">
			<xsl:with-param name="featureName" select="minLength" />
		</xsl:call-template>
