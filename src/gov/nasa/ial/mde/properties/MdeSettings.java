...
/**
 * MdeSettings manages initialization and storage of MDE description, sound, and
 * graphing properties (options), such as: line colors and thicknesses for
 ...
 * <p>
 * CartesianGraph properties and initial defaults are: <blockquote>axisColor =
 * Color.white; <br>
 ...
 * <blockquote>descriptionMode = "standards"; </blockquote> Sounder properties
 * and initial defaults are: <blockquote>negativeYValuesIndicator = HISS <br>
 ...
 * myMdeSettings = new MdeSettings(); </blockquote> MdeSettings will write the
 * properties to a file named MDE_Properties.properties (if the application/user
 ...
 * MdeSettings will attempt to initialize properties from this file. If the file
 * doesn't exist, MDE uses the default properties ("factory settings").
 * <p>
 * MDE will write to the properties file each time the application/user changes
 * a property value. This occurs automatically. The application does not need to
 * explicitly invoke a save with the saveParameters() method. <blockquote>
 * 
 * @version 1.0
 */
public class MdeSettings extends Settings {
	...
	// One of the values fom the INDICATOR_FREQUENCIES and INDICATOR_DURATIONS
	// arrays
	...
	// One of the values from the INDICATOR_FREQUENCIES and INDICATOR_DURATIONS
	// arrays
	...
	public static final String BUILD_NUMBER = "201207271407";
	...
	public static final String[][] PARAMETER_STRINGS = { { "a", "1" },
			{ "b", "1" }, { "c", "1" }, { "d", "1" },
			{ "e", "2.718281828459045" }, { "f", "1" }, { "g", "1" },
			{ "h", "0" }, { "k", "0" }, { "m", "1" } };
	...
	/**
	 * The string representation of the <code>NO_INDICATION</code> integer
	 * value.
	 */
	...
	/**
	 * The string representation of the <code>TAMBOR_CHANGE</code> integer
	 * value.
	 */
	 ...
	 * @see #VARIABLE
	 */
	public static final int[] BASIC_WAVEFORMS = { SINE, TRIANGLE, SAW, SQUARE,
			VARIABLE };
	...
	/**
	 * The integer values representing the negative y values indicators which
	 * are <code>HISS, TAMBOR_CHANGE</code>.
	 * 
	 * @see #HISS
	 * @see #TAMBOR_CHANGE
	 */
	 ...
	 */
	public static final String[] Y_AXIS_INDICATOR_STRINGS = { DING_STRING,
			NO_INDICATION_STRING };
	 ...
	 * @see #NO_INDICATION_STRING
	 */
	public static final String[] X_AXIS_INDICATOR_STRINGS = { CHIRP_STRING,
			NO_INDICATION_STRING };
	 ...
	 * @see #HIGH_STRING
	 */
	public static final String[] INDICATOR_FREQUENCY_STRINGS = { LOW_STRING,
			MEDIUM_STRING, HIGH_STRING };
	 ...
	 * @see #LONG_STRING
	 */
	public static final String[] INDICATOR_DURATION_STRINGS = { SHORT_STRING,
			MEDIUM_STRING, LONG_STRING };
	 ...
	 * @see #FAST_STRING
	 */
	public static final String[] TRACE_SWEEP_SPEED_STRINGS = { SLOW_STRING,
			MEDIUM_STRING, FAST_STRING };
	...
	private static final String SCALE_INDIVIDUAL_DATA_FOR_SONIFICATION_KEY = "mde.scale.individual.data.for.sonification";

	/** The SCALE INDIVIDUAL DATA FOR GRAPH property file key. */
	private static final String SCALE_INDIVIDUAL_DATA_FOR_GRAPH_KEY = "mde.scale.individual.data.for.graph";
	 ...
	 *            the properties filename to load the settings from.
	 */
	 ...
	 * traceOn = true; <br>
	 * </blockquote> Describer properties and initial defaults are:
	 * <blockquote>descriptionMode = "standards"; </blockquote> Sounder
	 * properties and initial defaults are: <blockquote>negativeYValuesIndicator
	 * = HISS <br>
	 * sonificationWaveform = TRIANGLE <br>
	 * traceSweepSpeed = MEDIUM <br>
	 * xAxisIndicator = NO_INDICATION <br>
	...
	protected void setDefaults(Properties defaults) {
		...
		defaults.put(BACKGROUND_COLOR_KEY,
		...
		defaults.put(DATA_POINT_COLOR_KEY,
				Integer.toString(dataPointColor.getRGB()));
		...
		defaults.put(SONIFICATION_WAVEFORM_KEY,
				Integer.toString(sonificationWaveform));

		defaults.put(Y_AXIS_INDICATOR_KEY, Integer.toString(yAxisIndicator));
		defaults.put(Y_AXIS_INDICATOR_FREQ_KEY,
				Integer.toString(yAxisIndicatorFreq));
		defaults.put(Y_AXIS_INDICATOR_DURATION_KEY,
				Integer.toString(yAxisIndicatorDuration));

		defaults.put(NEG_Y_VALUES_INDICATORS_KEY,
				Integer.toString(negYValuesIndicator));

		defaults.put(X_AXIS_INDICATOR_KEY, Integer.toString(xAxisIndicator));
		defaults.put(X_AXIS_INDICATOR_FREQ_KEY,
				Integer.toString(xAxisIndicatorFreq));
		defaults.put(X_AXIS_INDICATOR_DURATION_KEY,
				Integer.toString(xAxisIndicatorDuration));
	...
	protected void updateSettingsFromCachedProperties() {
		try {
			...
			tmp = properties
					.getProperty(SCALE_INDIVIDUAL_DATA_FOR_SONIFICATION_KEY);
			scaleIndividualDataForSonification = Boolean.valueOf(tmp)
					.booleanValue();
			...
			if (ArrayUtil.indexOfFirstMatch(sonificationWaveform,
					BASIC_WAVEFORMS) < 0) {
	...
	public String toString() {
		return "[" + axisColor + "," + backgroundColor + "," + lineColor + ","
				...
				+ scaleIndividualDataForSonification + " "
				+ scaleIndividualDataForGraph + " " + traceOn + " "
	 ...
	 * 
	 * @param showPoints
	 *            true to show show data points on the graph, false to not draw
	 *            them.
	 */
	 ...
	 * 
	 * @param enableAutoscale
	 *            true to automatically scale the graph, false for no graph
	 *            scaling.
	 */
	...
	/**
	 * @param scaleIndividualDataForSonification
	 *            the scaleIndividualDataForSonification to set
	 */
	...
	/**
	 * @param scaleIndividualDataForGraph
	 *            the scaleIndividualDataForGraph to set
	 */
	public void setScaleIndividualDataForGraph(
			boolean scaleIndividualDataForGraph) {
	 ...
	 * 
	 * @param flag
	 *            true to enable the trace, false to not show it.
	 */
	 ...
	 * 
	 * @param mode
	 *            the description mode is one of "visual" or "math".
	 */
	 ...
	 * 
	 * @param waveform
	 *            the sonification waveform, which is one of the values in
	 *            <code>BASIC_WAVEFORMS</code>.
	 * @see #BASIC_WAVEFORMS
	 */
	public void setSonificationWaveform(int waveform) {
		if (ArrayUtil.indexOfFirstMatch(waveform, BASIC_WAVEFORMS) < 0) {
			throw new IllegalArgumentException(
					"Sonification waveform must be one of the values in BASIC_WAVEFORMS");
		}
	 ...
	 * 
	 * @param indicator
	 *            the Y-axis indicator, which is one of the values in
	 *            <code>Y_AXIS_INDICATORS</code>.
	...
	public void setYAxisIndicator(int indicator) {
		if (ArrayUtil.indexOfFirstMatch(indicator, Y_AXIS_INDICATORS) < 0) {
			throw new IllegalArgumentException(
					"Y-axis indicator must be one of the values in Y_AXIS_INDICATORS");
		}
	 ...
	 * 
	 * @param duration
	 *            the Y-axis indicator duration, which is one of the values in
	 *            <code>INDICATOR_DURATIONS</code>.
	...
	public void setYAxisIndicatorDuration(int duration) {
		if (ArrayUtil.indexOfFirstMatch(duration, INDICATOR_DURATIONS) < 0) {
			throw new IllegalArgumentException(
					"Y-axis indicator duration must be one of the values in INDICATOR_DURATIONS");
		}
	 ...
	 * 
	 * @param indicator
	 *            the negative Y values indicator, which is one of the values in
	 *            <code>NEG_Y_VALUES_INDICATORS</code>.
	...
	public void setNegativeYValuesIndicator(int indicator) {
		if (!ArrayUtil.isCodeValid(indicator, NEG_Y_VALUES_INDICATORS)) {
			throw new IllegalArgumentException(
					"Negative Y-values indicator duration must be a combination of the values in NEG_Y_VALUES_INDICATORS");
		}
	 ...
	 * 
	 * @param frequency
	 *            the Y-axis indicator frequency, which is one of the values in
	 *            <code>INDICATOR_FREQUENCIES</code>.
	...
	public void setYAxisIndicatorFrequency(int frequency) {
		if (ArrayUtil.indexOfFirstMatch(frequency, INDICATOR_FREQUENCIES) < 0) {
			throw new IllegalArgumentException(
					"Y-axis indicator frequency must be one of the values in INDICATOR_FREQUENCIES");
		}
	 ...
	 * 
	 * @param indicator
	 *            the X-axis indicator, which is one of the values in
	 *            <code>X_AXIS_INDICATORS</code>.
	...
	public void setXAxisIndicator(int indicator) {
		if (ArrayUtil.indexOfFirstMatch(indicator, X_AXIS_INDICATORS) < 0) {
			throw new IllegalArgumentException(
					"X-axis indicator must be one of the values in X_AXIS_INDICATORS");
		}
	 ...
	 * 
	 * @param duration
	 *            the X-axis indicator duration, which is one of the values in
	 *            <code>INDICATOR_DURATIONS</code>.
	 * @see #INDICATOR_DURATIONS
	 */
	public void setXAxisIndicatorDuration(int duration) {
		if (ArrayUtil.indexOfFirstMatch(duration, INDICATOR_DURATIONS) < 0) {
			throw new IllegalArgumentException(
					"X-axis indicator duration must be one of the values in INDICATOR_DURATIONS");
		}
	 ...
	 * 
	 * @param frequency
	 *            the X-axis indicator frequency, which is one of the values in
	 *            <code>INDICATOR_FREQUENCIES</code>.
	 * @see #INDICATOR_FREQUENCIES
	 */
	public void setXAxisIndicatorFrequency(int frequency) {
		if (ArrayUtil.indexOfFirstMatch(frequency, INDICATOR_FREQUENCIES) < 0) {
			throw new IllegalArgumentException(
					"X-axis indicator frequency must be one of the values in INDICATOR_FREQUENCIES");
		}
	 ...
	 * 
	 * @param speed
	 *            the trace sweep speed, which is one of the values in
	 *            <code>TRACE_SWEEP_SPEEDS</code>.
	 * @see #TRACE_SWEEP_SPEEDS
	 */
	public void setTraceSweepSpeed(int speed) {
		if (ArrayUtil.indexOfFirstMatch(speed, TRACE_SWEEP_SPEEDS) < 0) {
			throw new IllegalArgumentException(
					"Trace sweep speed must be one of the values in TRACE_SWEEP_SPEEDS");
		}
