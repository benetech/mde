...
public class AnalyzedData implements AnalyzedItem, Cloneable {
	// Smallest fraction of display which can show an observable change -- 1
	// pixel
	...
	private Bounds preferredBounds = new Bounds(-DEFAULT_BOUND_VALUE,
			DEFAULT_BOUND_VALUE, DEFAULT_BOUND_VALUE, -DEFAULT_BOUND_VALUE);
	...
	/**
	 * Creates an instance of <code>AnalyzedData</code> using the specified X
	 * and Y-axis names and values.
	 * <p>
	 * The values of xData must be in ascending order and it is not checked.
	 * 
	 * @param xName
	 *            the name to use for the X-axis column of data.
	 * @param yName
	 *            the name to use for the Y-axis column of data.
	 * @param xData
	 *            the X-axis values.
	 * @param yData
	 *            the Y-axis values.
	 */
	public AnalyzedData(String xName, String yName, double[] xData,
			double[] yData) {
		if (xName == null) {
		...
		if (xData.length != yData.length) {
			throw new IllegalArgumentException(
					"X and Y data arrays are not the same length.");
		}
		if (xData.length <= 0) {
			throw new IllegalArgumentException(
					"X and Y data arrays must contain data.");
		}

		// DONE: Do we need to check to make sure the x-data is in ascending
		// order?
	 ...
	 * 
	 * @param index
	 *            the index into the X data array.
	 ...
	 * 
	 * @param index
	 *            the index into the Y data array.
	 * @return the value at the specified array index.
	 */
	 ...
	 * 
	 * @see
	 * gov.nasa.ial.mde.solver.symbolic.AnalyzedItem#computePoints(gov.nasa.
	 * ial.mde.math.Bounds)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see gov.nasa.ial.mde.solver.symbolic.AnalyzedItem#computePoints(double,
	 * double, double, double)
	 */
	public void computePoints(double left, double right, double top,
			double bottom) {
		...
		if (leftIndex > rightIndex) {
			throw new IllegalArgumentException(
					"Can not have left bound > right bound");
		}

		// The left and right indexes for all the real data points in the given
		// bounds.
		...
		// NOTE: Our current model is to either interpolate or decimate the real
		// data points.
		...
		if (totalPts < NUM_POINTS) {
			...
			points = PointsUtil.interpolatePoints(leftIndex, rightIndex,
					NUM_POINTS, xData, yData);
		} else if (totalPts > NUM_POINTS) {
			// Decimate the data to a length of NUM_POINTS.
			points = PointsUtil.decimatePoints(leftIndex, rightIndex,
					NUM_POINTS, xData, yData);
		} else {
		...
		// Create the array of x-values of the points for quick x-value to index
		// lookup.
		// We assume that the x values from the points array are sorted in
		// ascending order.
		if (points == null) {
			xPointValues = null;
			...
			for (int i = 0; i < len; i++) {
				...
				maximumYDisplayed = Math.max(points[i].yArray[0],
						maximumYDisplayed);
				minimumYDisplayed = Math.min(points[i].yArray[0],
						minimumYDisplayed);
			} // end for i
				// handle the case that the data is essentially constant
			if ((maximumYDisplayed - minimumYDisplayed) < AnalyzedData.MINIMUM_OBSERVABLE_FRACTION
					* (top - bottom)) {
				maximumYDisplayed = minimumYDisplayed
						+ AnalyzedData.MINIMUM_OBSERVABLE_FRACTION
						* (top - bottom);
			}
		}

		// Dispose of the current graph trails array so we don't have a memory
		// leak.
		disposeGraphTrails();
		...
		if (scaledGraphTrailsToFitBounds) {
			...
			for (int i = 0; i < len; i++) {
				MultiPointXY newPoint = new MultiPointXY(points[i].x, bottom
						+ f * (points[i].yArray[0] - minimumYDisplayed));
	 ...
	 * 
	 * @param scaledGraphTrailsToFitBounds
	 *            the scaledGraphTrailsToFitBounds to set
	 */
	public void setScaledGraphTrailsToFitBounds(
			boolean scaleGraphTrailsToFitBounds) {
	...
	/**
	 * @param scaledForSonification
	 *            the scaledForSonification to set
	 */
	...
	/**
	 * Returns the bounds of the data which represents the minimum and maximum X
	 * and Y values.
	 * 
	...
	public MultiPointXY getPoint(double position) {
		if ((points == null) || (points.length <= 0) || (position < 0.0)
				|| (position > 1.0)) {
			return null;
		}
	...
	public MultiPointXY getPoint(int index) {
		return ((points != null) && (index >= 0) && (index < points.length)) ? points[index]
				: null;
	}

	/**
	 * Returns the index in the array to the X-axis data value that is the
	 * closest to the specified x-value.
	 * 
	 * @param x
	 *            the X-axis value to find the index for.
	 * @return the index in the array to the X-axis data value that is the
	 *         closest to the specified x-value.
	 */
	public int getPointIndexNear(double x) {
		...
		if ((x < xPointValues[0])
				|| (x > xPointValues[xPointValues.length - 1])) {
			return -1;
		}
		...
		// Determine which of the two points the x-value is closer to the
		// specified x-value.
		if ((index > 0)
				&& (Math.abs(xPointValues[index - 1] - x) < Math
						.abs(xPointValues[index] - x))) {
			index--;
		}
	...
	/**
	 * Returns the real data point that has the specified x-value, otherwise
	 * return null.
	 * 
	 * @param x
	 *            the read data value to find the point for.
	 * @return the real data point that has the specified x-value, otherwise
	 *         return null.
	 */
	 ...
	 */
	public Object clone() {
		return new AnalyzedData(getXName(), getYName(), getXValues(),
				getYValues());
	}
	 ...
	 * 
	 * @return true if the specified object and this <code>AnalyzedData</code>
	 *         object are equal.
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof AnalyzedData) {
			...
			return (this.getYName().equals(ad.getYName())
					...
					&& Arrays.equals(this.yData, ad.yData) && Arrays.equals(
					this.xData, ad.xData));
		}
		return false;
	}
	...
	public String toString() {
		...
		strBuff.append(getClass().getName()).append("[").append("xDataName=")
				.append(xName).append(",").append("yDataName=").append(yName)
				.append(",").append("length=")
				.append((xData != null) ? xData.length : 0).append(",");
		if ((xData == null) || (yData == null)) {
			...
			for (int i = 0; i < len; i++) {
				...
				strBuff.append("(").append(xData[i]).append(",")
						.append(yData[i]).append(")");
			}
		}
		strBuff.append("]");
